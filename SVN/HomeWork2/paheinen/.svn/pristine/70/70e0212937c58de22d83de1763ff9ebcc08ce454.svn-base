...
Move Agent::nextMove() {
    ...
    for(auto i : moves){
        if((state.currentPlayer == 1) && ((i.to / 9) + (i.to % 9)) > ((currentMax.to / 9) + (currentMax.to % 9)))
        //if((state.currentPlayer == 1) && ((i.from / 9 - i.to / 9) + (i.from % 9 - i.to % 9)
        ...
        //if((state.currentPlayer == 1) && ((i.from / 9 - i.to / 9) + (i.from % 9 - i.to % 9)
         //< (currentMax.from / 9 - currentMax.to / 9) + (currentMax.from % 9 - currentMax.to % 9)))
        if((state.currentPlayer == 2) && ((i.to / 9) + (i.to % 9)) < ((currentMax.to / 9) + (currentMax.to % 9)))
        {
            Move &temp = i;
            //std::cout << "i: from " << i.from << ", to: " << i.to << std::endl;
            currentMax = temp;
